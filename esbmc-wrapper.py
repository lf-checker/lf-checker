...
def get_command_line(strat, prop, arch, benchmark, concurrency, dargs, concurrency_flag_predict):
  command_line = esbmc_path + dargs

  # Add benchmark
  command_line += benchmark + " "
  
  if concurrency_flag_predict:
    dargs = '--no-div-by-zero-check --force-malloc-success --no-align-check --no-unwinding-assertions '
    command_line = esbmc_path + dargs + benchmark + " "
  ...
  if concurrency:
    if not concurrency_flag_predict:
      command_line += " --no-por "
...
def verify(strat, prop, concurrency, dargs, concurrency_flag_predict):
  # Get command line
  esbmc_command_line = get_command_line(strat, prop, arch, benchmark, concurrency, dargs, concurrency_flag_predict)
  # Call ESBMC
...
parser.add_argument("-f", "--concurrencyFlagPredictor", help="Enable concurrency flags predictor")
#parser.add_argument("model", nargs='?', help="Path to the flag predictor")
...
concurrency_flag_predict = args.concurrencyFlagPredictor
#ml_model = args.model
if version:
